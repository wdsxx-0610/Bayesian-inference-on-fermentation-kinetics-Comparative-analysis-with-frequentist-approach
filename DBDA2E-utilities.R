...
saveGraph = function(file = "saveGraphOutput", type = "pdf", ...) {
  ...
  if (capabilities("png") && (dev.cur() == 1 || names(dev.cur()) == "RStudioGD")) {
    ...
    if (type %in% c("png", "jpeg", "jpg", "tiff", "bmp")) {
      sptype <- ifelse(type == "jpg", "jpeg", type)
      dev.copy(get(sptype), file = file, ...)
      dev.off()
      return(invisible())
    }
...
diagMCMC = function( codaObject , parName=varnames(codaObject)[1] ,
  ...
  if ( !is.null(saveName) ) {
    saveGraph( file=paste0(saveName,"Diag_",parName), type=saveType)
  }
}
